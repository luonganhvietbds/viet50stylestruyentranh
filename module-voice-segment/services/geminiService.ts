...
export const INVALID_KEY_EVENT = 'INVALID_GEMINI_KEY';

const getAi = () => {
    // BYOK: Get key from local storage
    const apiKey = localStorage.getItem(API_KEY_STORAGE_KEY);

    if (!apiKey) {
        // Dispatch event to open modal
        window.dispatchEvent(new Event(INVALID_KEY_EVENT));
...
const handleApiError = (error: any) => {
    ...
    // Check for 400 (Bad Request - sometimes invalid key), 401 (Unauthorized), 403 (Forbidden)
    // or specific error messages from the SDK
    if (status === 401 || status === 403 || errorMsg.includes('api key') || errorMsg.includes('unauthenticated') || errorMsg.includes('permission denied')) {
        // Clear bad key
...
export const getAdvancedSuggestions = async (
  ...
  try {
    ...
    return Array.isArray(suggestions) && suggestions.length > 0 ? suggestions : ["AI could not provide suggestions, please try again."];
  } catch (error: any) {
...
export const bulkFixSegments = async (segments: Segment[], minSyllables: number, maxSyllables: number): Promise<Segment[]> => {
    try {
        const ai = getAi();
        ...
        const jsonText = response.text.trim();
        const fixedSegmentsData = JSON.parse(jsonText) as {segment_id: string; text: string}[];
...
export const mergeSegmentsWithAI = async (segment1: Segment, segment2: Segment, minSyllables: number, maxSyllables: number): Promise<string> => {
    try {
        const ai = getAi();
        const prompt = `
        ...
        const mergedText = response.text.trim();
        return mergedText || `${segment1.text} ${segment2.text}`; // Fallback if AI returns empty
    } catch (error: any) {
        handleApiError(error);
...
export const getTargetedSuggestion = async (
    currentSegment: Segment,
    prevSegment: Segment | null,
    nextSegment: Segment | null,
